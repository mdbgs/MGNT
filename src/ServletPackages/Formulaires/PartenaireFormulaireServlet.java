...
public class PartenaireFormulaireServlet extends GetAuthorisationUsers implements NumericConstant {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		this.doGet(request, response, "responsable_Suivi_Evaluation", "Formulaire/Partenaire.jsp", connection);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String valuePartenaire = "'%'"+password+"'%" + dateNow + "%'inconnu'%";
		// ConnexionServlet servlet = new ConnexionServlet();

		try {
			connection = this.getDataSource().getConnection();
			int rs = ComputeQueryBean.insertInAllTable(pseudo + valuePartenaire, "compte", connection);
			// int number = 0;
			// while(rs==0){
			// number++;
			// pseudo+=number;
			// rs = ComputeQueryBean.insertDatabase(pseudo+valuePartenaire,
			// "compte",connection);
			// }
			date = dateFormat.parse(dateDeSignature);
			date = dateFormat.parse(expirationDeLaccord);
			valuePartenaire = "%" + pseudo + "'%" + "'" + nom + "'%" + "'" + type + "'%" + "'" + adresse + "'%" + "'"
					+ telephone + "'%" + "'" + email + "'%" + "'" + boitePostale + "'%" + "'" + titreAcc + "'%" + "'"
					+ domaineDeCollaboration + "'%" + dateFormat.format(date) + "%" + dateFormat.format(date) + "%"
					+ "'" + resultatAttendu + "'%" + "'" + projetConjoint + "'%";
			rs = ComputeQueryBean.insertInAllTable(valuePartenaire, "Partenaires", connection);
